    ...
    <div>
      ...
      {videos.map((el,index) => { 
        return <div className='youtube-video-card' style={{display: 'flex', alignItems: 'center', marginLeft: '20px'}} >
          <img style={{borderRadius: '20px' ,width: '200px', height: '200px', maxWidth: 'auto', cursor: 'pointer'}} src={el.video.thumbnails[0].url} alt={el.video.title} onClick= {() => {window.open(`https://www.youtube.com/watch?v=${el.video.videoId}`,'_blank')}}/>
